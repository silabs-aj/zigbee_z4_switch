...
void bootloader_rebootAndInstall(void)
{
  ...
  if (info.capabilities & BOOTLOADER_CAPABILITY_EM4_GPIO_RETENTION) {
    // Configure BURTC for EM4 wakeup
    configureBurtcAndEnterEM4();
  } else {
...
static int32_t btl_em4GpioRetentionConfigSave(const gpio_em4_retention_t* pins, uint8_t num_pins)
{
  // Check if EM4 GPIO retention capability is present
  if (pins == NULL || num_pins == 0 || num_pins > MAX_EM4_RETAINED_PINS) {
    return BOOTLOADER_ERROR_GPIO_RETENTION_INVALID;
  }

  gpio_em4_retention_config_t config;
  config.header.fields.signature = BURAM_SIGNATURE_VALID;
  config.header.fields.reset_reason = BURAM_RESET_REASON_BOOTLOAD;    // Initialize reset reason to BOOTLOAD
  config.header.fields.reset_counter = 0;   // Initialize reset counter to 0
  config.header.fields.num_pins = num_pins;
  ...
  // Clear unused pins
  for (uint8_t i = num_pins; i < MAX_EM4_RETAINED_PINS; i++) {
    config.pins[i].packed = 0;
  }

  // Calculate the total bytes to write (header + used pins)
  size_t total_bytes = sizeof(config.header) + num_pins * sizeof(gpio_em4_retention_t);
  const uint8_t *src = (const uint8_t *)&config;
  size_t num_registers = (total_bytes + 3) / 4;
  ...
  for (size_t i = 0; i < num_registers; ++i) {
    ...
    for (size_t j = 0; j < 4; ++j) {
      size_t idx = i * 4 + j;
      if (idx < total_bytes) {
        value |= ((uint32_t)src[idx]) << (8 * j);
      }
    }
    BURAM->RET[i].REG = value;
  }

  // Configure EM4 GPIO retention mode to SWUNLATCH
...
int32_t bootloader_rebootAndInstallWithGpioRetention(const gpio_em4_retention_t* pins, uint8_t num_pins, bool auto_configure_wakeup)
{
  ...
  if (info.capabilities & BOOTLOADER_CAPABILITY_EM4_GPIO_RETENTION) {
    // Validate GPIO configuration parameters
    if (pins == NULL || num_pins == 0 || num_pins > MAX_EM4_RETAINED_PINS) {
      // Invalid parameters, return error
      return BOOTLOADER_ERROR_GPIO_RETENTION_INVALID;
    }

    // Save GPIO configuration to BURAM
    int32_t result = btl_em4GpioRetentionConfigSave(pins, num_pins);
    if (result != BOOTLOADER_OK) {
      // GPIO retention failed, return error
      return result;
    }

    // Based on auto_configure_wakeup parameter, either configure BURTC or enter EM4 directly
    if (auto_configure_wakeup) {
      // Configure BURTC for EM4 wakeup and enter EM4
      configureBurtcAndEnterEM4();
    } else {
      // Enter EM4 directly, leaving wakeup mechanism to application
      EMU_EnterEM4();
    }

    // Note: The following code will never be reached as the system enters EM4 sleep mode
    // This is included for completeness and to satisfy the return type requirement
    return BOOTLOADER_OK;
  } else {
    // EM4 GPIO retention not available, return error
    return BOOTLOADER_ERROR_GPIO_RETENTION_INVALID;
  }
}
