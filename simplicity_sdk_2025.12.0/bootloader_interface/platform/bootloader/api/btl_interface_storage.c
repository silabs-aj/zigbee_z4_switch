...
static int32_t validateStorageSlot(uint32_t slotId,
                                   uint32_t offset,
                                   size_t length,
...
{
  int32_t retVal;
  // Check for the storage type of the device
  BootloaderStorageInformation_t storageInfo;
  bootloader_getStorageInfo(&storageInfo);

  if (storageInfo.storageType == INTERNAL_FLASH) {
    // Validate the storage slot
    BootloaderStorageSlot_t storageSlot;
    retVal = validateStorageSlot(slotId, offset, length, &storageSlot);
    if (retVal != BOOTLOADER_OK) {
      return retVal;
    }

    // Perform the read operation
    retVal = bootloader_readRawStorage(storageSlot.address + offset, buffer, length);
  }
                                  ...
                                  size_t   length)
{
  ...
  if (storageInfo.storageType == INTERNAL_FLASH) {
    retVal = storage_readRaw(address, buffer, length);
                                   ...
                                   size_t   length)
{
  ...
  if (storageInfo.storageType == INTERNAL_FLASH) {
    retVal = storage_writeRaw(address, buffer, length);
    if (retVal != BOOTLOADER_OK) {
      return retVal;
    }
                                   ...
                                   size_t   length)
{
  ...
  if (storageInfo.storageType == INTERNAL_FLASH) {
    retVal = storage_eraseRaw(address, length);
